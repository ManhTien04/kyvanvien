...
class HistoryGift extends StatefulWidget {
  @override
  _HistoryGiftState createState() => _HistoryGiftState();
}
...
class _HistoryGiftState extends State<HistoryGift> {
  ...
  void initState() {
    ...
    _initializeUserId().then((_) {
      if (userId != null) {
        fetchHistoryGifts(userId!).then((historyGifts) {
          setState(() {
            transactions = historyGifts;
          });
        });
      }
    });
  ...
  Future<List<HistoryModel>> fetchHistoryGifts(int userId) async {
    final response = await http.get(Uri.parse('http://10.0.2.2:8080/api/v1/history-gift/user/$userId'));

    if (response.statusCode == 200) {
      List jsonResponse = json.decode(response.body);
      return jsonResponse.map((item) => HistoryModel.fromJson(item)).toList();
    } else {
      throw Exception('Failed to load history gifts from API');
    }
  }
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return TransactionItem(
            date: DateFormat('dd/MM/yyyy').format(transactions[index].createAt),
            amount: transactions[index].amount.toString(),
            description: Utf8Decoder().convert(transactions[index].storyName.codeUnits),
            time: DateFormat('HH:mm').format(transactions[index].createAt),
          );
        },
      ),
      backgroundColor: primaryColor,
